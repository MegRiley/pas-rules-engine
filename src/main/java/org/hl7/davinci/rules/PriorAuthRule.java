...
public class PriorAuthRule {
     ...
     */
    public static FhirUtils.Disposition computeDisposition(Bundle bundle, int sequence) {
        ...
        FhirUtils.Disposition disposition;
        if (elmFile == null) {

            disposition = FhirUtils.Disposition.PENDING;
        } else {
            ...
            if (executeRule(context, Rule.GRANTED))
                disposition = FhirUtils.Disposition.GRANTED;
            else if (executeRule(context, Rule.PENDED))
                disposition = FhirUtils.Disposition.PENDING;
            else
                disposition = FhirUtils.Disposition.DENIED;
        }
                    ...
                    for (File file : topic.listFiles()) {
                        ...
                        if (file.getName().equalsIgnoreCase("TopicMetadata.json")) {
                            try {
                                ...
                                // Add each system/code pair to the database
                                for (Mapping mapping : metadata.getMappings()) {
                                    for (String code : mapping.getCodes()) {
                                        ...
                                        if (!App.getDB().write(Database.Table.RULES, dataMap))
                                            return false;
                                    }
                                }
                            } catch (Exception e) {
    ...
    private static boolean executeRule(Context context, Rule rule) {
        ...
        try {
            ...
            // "Rule " + rule.value() + " did not return a boolean (Returned value: " +
            // rawValue.toString() + ")",
            // e);
            return false;
        }
    ...
    private static String getRuleFileFromItem(ItemComponent claimItem) {
        ...
        String topic = App.getDB().readString(Database.Table.RULES, constraintParams, "topic");
        String rule = App.getDB().readString(Database.Table.RULES, constraintParams, "rule");
